...
class Setup(smach.State):
    ...
    def execute(self,userdata): # Userdata es informacion que se puede mover entre estados
        ...
        self.tts.say("Inicio prueba de deteccion de q r")
...
class Example(smach.State):
    ...
    def execute(self,userdata): # Userdata es informacion que se puede mover entre estados
        ...
        self.tts.say("Ahora levante q r")
        self.tts.wait_until_done()
        time.sleep(2)
        ...
        for i in range(3):
            ...
            if len(detections.labels)>0:
                self.tts.say("Encontrada, {}".format(detections.labels[0]))
                self.tts.wait_until_done()
                return "succeeded"
...
def getInstance(robot):
    ...
    with sm:
        ...
        smach.StateMachine.add('INSTRUCTIONS',Speak(robot,"Cuando se le indique levante paleta con q r "),
            transitions={
                'succeeded':'EXAMPLE1'
            }
        )
