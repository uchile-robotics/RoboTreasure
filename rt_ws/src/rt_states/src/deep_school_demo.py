...
def getInstance(robot):
    ...
    with sm:
        ...
        smach.StateMachine.add('INSTRUCTIONS_ANSWER',Speak(robot,"Ahora se probara seleccion por brazo. Levante brazos para responder cuando se indique"),
            transitions={
                'succeeded':'SELECTION_EXAMPLE1'
            }
        )
        smach.StateMachine.add('SELECTION_EXAMPLE1',AnswerSelection(robot),
            transitions={
                'succeeded':'SAY_ANSWER1'
            },

        )
        smach.StateMachine.add('SAY_ANSWER1',SaySelection(robot),
            transitions={
                'succeeded':'SELECTION_EXAMPLE2'
            },

        )
        smach.StateMachine.add('SELECTION_EXAMPLE2',AnswerSelection(robot),
            transitions={
                'succeeded':'SAY_ANSWER2'
            },

        )
        smach.StateMachine.add('SAY_ANSWER2',SaySelection(robot),
            transitions={
                'succeeded':'SELECTION_EXAMPLE3'
            },

        )
        smach.StateMachine.add('SELECTION_EXAMPLE3',AnswerSelection(robot),
            transitions={
                'succeeded':'SAY_ANSWER3'
            },

        )
        smach.StateMachine.add('SAY_ANSWER3',SaySelection(robot),
            transitions={
                'succeeded':'FINISH'
            },
